module addr::rotate {
    ...
    public entry fun create_resource_account(account: &signer, auth_key: vector<u8>) acquires ResourceAccountData {
        resource_account::create_resource_account(account, x"1001", auth_key);
        let resource_account_address = account::create_resource_address(&signer::address_of(account), x"1001");
    ...
    public entry fun create_resource_account_and_claim_signer_cap(account: &signer) acquires ResourceAccountData, SignerCap {
        let (resource_account_signer, resource_signer_cap) = account::create_resource_account(account, x"1001");
    ...
    public entry fun offer_signer_cap(account: &signer, addr: address, signer_capability_sig_bytes: vector<u8>, account_public_key_bytes: vector<u8>) {
        // let resource_account_data = borrow_global<ResourceAccountData>(@addr);
        // let resource_address = *table::borrow(&resource_account_data.resource_account, signer::address_of(account));

        // let resource_signer_cap_data = borrow_global_mut<SignerCap>(@addr);
        // let resource_signer = table::borrow(&mut resource_signer_cap_data.cap, resource_address);

        account::offer_signer_capability(account, signer_capability_sig_bytes, 0, account_public_key_bytes, addr);
    }
    ...
    public entry fun retrive_signer_cap(account: &signer) acquires ResourceAccountData, SignerCap {
        ...
        let signer_cap = resource_account::retrieve_resource_account_cap(&resource_signer, resource_address);
